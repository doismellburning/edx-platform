...
class XQueueCertInterfaceAddCertificateTest(ModuleStoreTestCase):
    ...
    def test_add_cert_with_honor_certificates(self, mode):
        ...
        # Verify that the task was sent to the queue with the correct template
        actual_body = json.loads(kwargs['body'])
        self.assertIn(
            'certificate-template-{id.org}-{id.course}.pdf'.format(
                id=self.course.id
            ),
            actual_body['template_pdf']
        )

        self.assert_queue_response(kwargs['header'], mode)
    ...
    def test_add_cert_with_verified_certificates(self, mode):
        ...
        actual_body = json.loads(kwargs['body'])
        self.assertIn(
            'certificate-template-{id.org}-{id.course}-verified.pdf'.format(
                id=self.course.id
            ),
            actual_body['template_pdf']
        )
        self.assert_queue_response(kwargs['header'], "verified")

    def assert_queue_response(self, header, mode):
        actual_header = json.loads(header)
        self.assertIn('https://edx.org/update_certificate?key=', actual_header['lms_callback_url'])
        certificate = GeneratedCertificate.objects.get(user=self.user_2, course_id=self.course.id)
        self.assertEqual(certificate.mode, mode)
